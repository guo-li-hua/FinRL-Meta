...
from talib._ta_lib import MACD, RSI, CCI, DX
...
p = DataProcessor(
    data_source='binance',  # 'tushare',  # "binance",
...
p.run_download(ticker_list, cache=True)
p.clean_data()
df = p.dataframe
print(f"p.dataframe: {p.dataframe}")


# df.to_csv('./cache/dataset.csv', index=False)

# read data
# p.dataframe = pd.read_csv('./cache/dataset.csv')
# df = p.dataframe
# print(df)
...
pro_df = add_technical_indicator(df, technical_indicator_list)
print(f"pro_df: {pro_df.head()}")
...
# processed_df = pro_df
processed_df = p.add_technical_factor_with_data(pro_df, factor_list)
processed_df.to_csv("./cache/add_technical_factor_with_data_1.csv", index=False)
# 丢弃前n个数据，避免数据为0影响训练结果
processed_df = processed_df.iloc[40:, :].reset_index(drop=True)
print(f"processed_df: {processed_df.head()}")
# print(f"processed_df: {processed_df}")
processed_df.to_csv("./cache/add_technical_factor_with_data_2.csv", index=False)
...
def get_vol(prices, span=100):
    ...
    # 2. estimate rolling standard deviation  评估滚动标准差
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.state_dim = state_dim  # all the features
